class SchemesSurveysController < ApplicationController
    ...
    def edit
        ...
        if @farm.schemes_survey
            @schemes_survey = @farm.schemes_survey
        else
            @schemes_survey = @farm.build_schemes_survey
        end
    end
    ...
    def update
        ...
        if params[:creation]
            redirect_to farm_creation_path(:countryside_stewardship_survey_details, farm_id: @farm.id)
        else
